...
public enum TerminalLocator {
    ...
    ///   should treat this as "snapshot unavailable, do not draw any
    ///   conclusions" — never as "no claudes are running".
    /// - empty dict when `ps` succeeds but there are zero claude processes.
    ///   Callers can act on this (e.g., the sweep prunes everything).
    ///
