...
- **NotchShape**（`#64`）: 平顶 + 顶部外角向屏幕边缘外扩 + 圆底的灵动岛轮廓（照搬 DynamicNotchKit / boring.notch），真刘海与模拟刘海共用（模拟侧用 `init(cornerRadius:)` 退化成纯圆底，保持原样）。紧凑态通过 `CenteredNotchLayout` 把运行时 `notchSize.width` 禁入区严格锚定屏幕中心；左右内容各占相同的最大测量宽度，因此状态图标、百分比位数和动态额度窗口不会把文字推入物理刘海（#180）。
...
| `NotchCompactView` | `Sources/AppLib/Notch/NotchCompactView.swift` | 折叠 / 紧凑状态视图；固定宽状态位按错误（红）→待用户（黄）→工作/空闲排序，多项注意事件显示数量；配额窗口由 `QuotaWindowPresentation` 动态选择，单独 7d 时直接提升为主 chip，不再显示不存在的 `5h —` |
...
| `SimulatedNotchView` | `Sources/AppLib/SimulatedNotch/SimulatedNotchView.swift` | Compact / hoverWide 内容（实际可用的 5h/7d 配额 + NotchShape）。读 `NotchModeStore.compactAgent` 决定显示哪个 agent 的配额；单窗口时不渲染另一个空槽 |
| `SimulatedNotchFullView` | `Sources/AppLib/SimulatedNotch/SimulatedNotchFullView.swift` | Full 模式：动态额度进度条 header + 滚动 session 列表。当两 agent 都有数据时，header 按两边窗口并集自动左右切割（左 Claude / 右 Codex）；gear 跟随第一条可见额度，无窗口时保留独立状态行 |
