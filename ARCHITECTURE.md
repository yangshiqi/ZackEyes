...
| compact 生命周期 UI (#37) | `Sources/AppLib/Notch/NotchExpandedView.swift` (`compactMarker`) | 压缩期间卡片显示 `Compacting context…` + trigger（manual/auto），完成后 60s 内显示 `Context compacted ×N`。**存在的理由**：压缩期间 agent 不产出、没有工具在跑，看起来和"卡死"一模一样——这是 issue 的核心验收项。`SessionInfo.compactCount` / `lastCompactedAt` **跨回合累积**（描述的是会话，不是回合），与 `compactTrigger`（#181，每个回合边界都清）刻意不同。两条完成路径共用 `recordCompactFinished()`：真 `PostCompact` 事件，和 #186 给交互式 CC 的 StatusLine 骤降推断（`clearCompactMarker`）——否则计数会随用户处于哪种模式而不同。 |
| `SubagentBadge` | `Sources/AppLib/Notch/SubagentBadge.swift` | #40/#79 Claude subagent 在飞标记。单个优先显示**描述**（`Locate the theme color sourc…`，28 字符截断），无描述时回退类型（`Explore`），再回退 `1 agent`；多个显示计数（各自描述不同、都塞不下），明细进 tooltip。`label(for:)` 纯函数。 |
...
| `JumpDiagnostics` | `Sources/AppLib/Terminal/JumpDiagnostics.swift` | #42 跳转失败归因。`activateTerminal` 的返回值以前被丢弃，点了没反应与「点击没注册」无法区分。`classify(hadPid:hadCwd:accessibilityTrusted:)` 是纯函数，按**可操作性**排序三种结局：`noTarget`（既无 pid 又无 cwd，无事可做）→ `accessibilityDenied`（用户去授权即可修）→ `noMatchingWindow`（终端已关，只能告知）。**刻意不做重试队列**：跳转是同步的用户手势，失败原因都不是「等一会儿会好」，重试只会推迟反馈。徽标走 `recentlyFailedJump` 短时窗，与 #37 压缩标记同一套 transient 机制。 |
