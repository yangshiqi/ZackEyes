...
| `SubagentBadge` | `Sources/AppLib/Notch/SubagentBadge.swift` | #40 Claude subagent 在飞标记。单个显示类型（`Explore` 比 `1 agent` 信息量大且同宽），多个显示计数（fan-out 常是同类型，也塞不下）。`label(for:)` 纯函数。 |
