...
## 3.5 日志语言可指定（✅ 新增需求 2026-07-30）

日志语言是设置项，不跟随系统语言，也不跟随会话语言 —— 用户可能用中文和 agent 对话但想要英文日志（或反过来）。

影响三处，第三处不明显：

| 层 | 影响 |
|----|------|
| `JournalDistiller` | prompt 里的「用中文」变成参数 |
| `JournalRenderer` | 脚手架文案（`## Lessons`、`N projects · N sessions`、outcome 词）跟随所选语言 |
| **`JournalSanitizer`** | **字符白名单必须包含该语言的字集** |

**加一门语言不是加一个 prompt 字符串。** 白名单当前只放行 CJK / 假名 / 拉丁，所以选韩语（Hangul U+AC00–D7AF 不在白名单里）会导致**整份日志被逐条丢弃**，且是静默的。

而且每门语言都需要自己的假阳性语料 —— 证据是中文那个高熵误杀：三轮评审加一套假阳性语料都没抓到，`三`/`十`/`百` 在 Swift 里 `isNumber == true` 且中文无词边界，两个事实叠起来才炸。别的语言会有别的、同样不明显的组合。

**V1 只提供 中文 / English / 日本語** —— 这三种的字集已在白名单内且有语料覆盖。其余语言按「加字集 + 加语料 + 加回归测试」逐个开，不做「反正白名单放行就行」的泛化。

**白名单不随所选语言收窄**，永远是所有受支持字集的并集：中文开发者的工作记录里天然混着英文术语（本文的语料里就有 `ZackEyes`、`GitHub`），收窄会立刻制造误杀。
