...
用户试听了 11 个候选片段，删掉了 3 个（smooth-operator、leave-me-alone、xiaoxin-nose），
本设计只实现留下的 8 个中未落地的部分。
...
`f1` 音效从 7 个增加到 10 个（含 None）。追加在 `f1-radio` 之后、`None` 之前，
...
```swift
case shinchan   // displayName: "蜡笔小新"
```

### 音效（7 + None）

配音版本要求**国语新版**（2019 年后台配班底）。因为无法靠听辨版本，
判定依据是片源标注：素材取自明确标注「蠟筆小新新番 第十季 / 第十一季」的两集
...
正片音轨自带 BGM，不像专门制作的音效那样纯净——这是从剧集取材的固有代价，已经用户试听确认可接受。
