...
| D5 | **三档详细程度（简洁 / 适中默认 / 详细）** | 三档**只调长度，不调粒度**；粒度红线全局固定。⚠️ **默认档 2026-07-30 由「简洁」改为「适中」**，依据见 §5.2 |
...
| D11 | **日志语言可指定**（2026-07-30 新增） | 不跟随系统语言，也不跟随会话语言 —— 用中文和 agent 对话但想要英文日志是合理诉求。⚠️ 加一门语言**不是加一个 prompt 字符串**，见 §5.5 |
...
|------|---------|--------|----------------|
| 简洁 | ≤ 200 | 1 句 | ≤ 3 条 / ≤ 40 |
| **适中（默认）** | ≤ 500 | 2–3 句 | ≤ 5 条 / ≤ 80 |
| 详细 | ≤ 1200 | 一小段 | ≤ 8 条 / ≤ 150 |

**默认档改为「适中」（2026-07-30）**：依据是第一份手工渲染的真实日报。「简洁」档每项目只留 1 句，而实测那天单个项目里做了三件互不相干的事 —— 压缩成一句就必须丢掉两件，而「今天在这个项目上推进了哪几件事」恰恰是回看时最想知道的。原稿选「简洁」是在没有任何真实产物时下的判断，看到产物后改掉。
...
### 5.5 日志语言（D11，2026-07-30 新增）

设置项，默认跟随**中文**（本产品当前用户），可选 English / 日本語。不跟随系统语言，也不跟随会话语言。

影响三层，第三层不明显：

| 层 | 影响 |
|----|------|
| `JournalDistiller` | prompt 里的语言指令变成参数 |
| `JournalRenderer` | 脚手架文案（`## Lessons`、`N projects · N sessions`、outcome 词）跟随 |
| **`JournalSanitizer`** | **字符白名单必须包含该语言的字集** |

**加一门语言不是加一个 prompt 字符串。** 白名单当前只放行 CJK / 假名 / 拉丁；选一门字集不在其中的语言（如韩语 Hangul U+AC00–D7AF），结果是**整份日志被逐条丢弃，且静默** —— 用户只会看到空日报。

每门语言还需要自己的假阳性语料。依据是 2026-07-30 抓到的中文误杀：三轮评审加一套已有的假阳性语料都没发现它，因为要两个都不显眼的事实叠起来才触发 —— 中文无词边界（整句被当成一个 token），且 Swift 的 `isNumber` 对中文数字 `三`/`十`/`百` 返回 true，于是「含中文数字且 ≥16 字」的任何中文句子都被判成高熵密钥串。**别的文字会有别的、同样不明显的组合**，不能靠推理免测。

因此 **V1 只提供中文 / English / 日本語**（字集已在白名单内且有语料覆盖），其余按「加字集 + 加语料 + 加回归测试」逐门开，不做「反正白名单放行就行」的泛化。

**白名单不随所选语言收窄**，永远是全部受支持字集的并集：中文开发者的工作记录天然混着英文术语，收窄会立刻制造误杀。
